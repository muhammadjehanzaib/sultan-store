...
export async function GET(
  ...
  try {
    ...
    // Generate HTML invoice (same as customer invoice but with admin branding)
    const invoiceHTML = generateAdminInvoiceHTML(order);
...
function generateAdminInvoiceHTML(order: any): string {
...
<head>
    ...
    <title>Admin Invoice - Order #${order.id}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <table class="items-table">
        ...
        <tbody>
            ${order.items.map((item: any) => {
              ...
              return `
                <tr>
                    <td><small style="color: #666;">${item.product?.id || 'N/A'}</small></td>
                    <td>
                        <div>
    ...
    <div class="no-print">
        ...
        <button class="print-btn" onclick="history.back()" style="background: #6b7280;">Back to Admin</button>
    </div>
    
    <div class="footer">
        <p>Generated by Admin Panel on ${new Date().toLocaleDateString()}</p>
        <p>Internal document - For administrative use only</p>
    </div>
